...
describe('Ride-My-Way Rides Test', () => {
    ...
    it('should return error if login as User and destination start point is empty', (done) => {
        request(app)
            ...
            .end((error, res) => {
                expect(400);
                expect(res.body.errors.startPoint[0]).to.include('destination starting point is required');
                if(error) done(error);
                done();
            });
    });
    it('should return error if destination stop point is empty', (done) => {
        request(app)
            ...
            .end((error, res) => {
                ...
                expect(res.body.errors.stopPoint[0]).to.include('destination stopping point is required');
                if(error) done(error);
                done();
            });
    });
    it('should return error if departure time is empty', (done) => {
        request(app)
            ...
            .end((error, res) => {
                expect(400);
                expect(res.body.errors.departureTime[0]).to.include('departure time is required');
                if(error) done(error);
                done();
            });
    });
    it('should return error if departure Date is empty', (done) => {
        request(app)
            ...
            .end((error, res) => {
                expect(400);
                expect(res.body.errors.departureDate[0]).to.include('departure date is required');
                if(error) done(error);
                done();
            });
    });
